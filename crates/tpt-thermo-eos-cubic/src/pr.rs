...
/// both can be overridden via [`PengRobinson::with_mixing`] /
/// [`PengRobinson::with_alpha`].
...
impl PengRobinson {
    ...
    /// parameters (opt-in; use [`PengRobinson::from_database`] for the zero-BIP
    /// default).
    ...
    pub fn with_mixing(
        ...
        let inner = CubicEos::from_database(CubicModel::PengRobinson, db, soave(), mixing)?;
        Ok(Self { inner })
    }
    ...
    pub fn with_alpha_and_mixing(
        ...
        let inner = CubicEos::from_database(CubicModel::PengRobinson, db, alpha, mixing)?;
        Ok(Self { inner })
    }
